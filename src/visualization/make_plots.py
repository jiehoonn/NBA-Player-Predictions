...
def plot_performance_comparison(metrics_dir, output_dir):
    ...
    ax1.bar(x - width/2, df_metrics['Model MAE'], width, label='Model', color='steelblue')
    ax1.bar(x + width/2, df_metrics['Baseline MAE'], width, label='Baseline (5-game avg)', color='lightcoral')
    ...
    ax2.barh(df_metrics['Target'], df_metrics['Improvement (%)'], color=colors, alpha=0.7)
...
def plot_feature_importance(model_path, output_dir, top_n=15):
    ...
    plt.title(f'Top {top_n} Features - {model_data["target"]} ({model_data["model_name"]})')
...
def plot_predictions_vs_actual(model_path, data_path, output_dir):
    ...
    ax1.set_title(f'{target} Predictions vs Actual ({model_data["model_name"]})\nMAE: {mae:.2f}, RMSE: {rmse:.2f}, R²: {r2:.3f}')
    ax1.legend()
    ax1.grid(alpha=0.3)
...
def plot_error_distribution(model_path, data_path, output_dir):
    ...
    plt.axvline(x=errors.mean(), color='green', linestyle='--', linewidth=2, label=f'Mean Error: {errors.mean():.2f}')
    plt.xlabel('Prediction Error (Actual - Predicted)')
    plt.ylabel('Frequency')
    plt.title(f'Error Distribution - {target} ({model_data["model_name"]})\nMAE: {abs(errors).mean():.2f}, Std: {errors.std():.2f}')
    plt.legend()
...
def plot_time_series_predictions(model_path, data_path, output_dir, player_name=None, n_games=50):
    """
    ...
    plt.plot(game_numbers, y_actual.values, 'o-', label='Actual', linewidth=2, markersize=6, color='steelblue')
    plt.plot(game_numbers, y_pred, 's--', label='Predicted', linewidth=2, markersize=6, color='coral')

    player_name_str = player_df.iloc[0]['PLAYER_NAME'] if 'PLAYER_NAME' in player_df.columns else 'Unknown'
...
def main():
    parser = argparse.ArgumentParser(description='Generate visualizations for model evaluation')
    parser.add_argument('--models', default='artifacts/models/', help='Directory containing trained models')
    parser.add_argument('--data', default='data/processed/features_enhanced_3seasons.parquet', help='Processed features file')
    parser.add_argument('--output', default='reports/figures/', help='Output directory for plots')
    parser.add_argument('--plots', nargs='+',
                       choices=['performance', 'importance', 'predictions', 'errors', 'timeseries', 'all'],
                       default=['all'],
    ...
    if 'all' in plots_to_generate:
        plots_to_generate = ['performance', 'importance', 'predictions', 'errors', 'timeseries']
