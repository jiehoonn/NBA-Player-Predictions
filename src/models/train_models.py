...
def train_model(train_df, val_df, test_df, target, features):
    ...
    if model_name == 'Lasso':
        ...
        X_train_proc, X_val_proc, X_test_proc = X_train.values, X_val.values, X_test.values
    ...
    for split_name, X_split, y_split in [('train', X_train_proc, y_train),
...
def main():
    parser = argparse.ArgumentParser(description='Train best-performing models for NBA predictions')
    parser.add_argument('--input', required=True, help='Input features parquet file')
    parser.add_argument('--target', required=True, choices=['PTS', 'REB', 'AST'], help='Target variable')
    parser.add_argument('--output', default='artifacts/models/', help='Output directory')
    parser.add_argument('--features', choices=['original', 'all'], default='all',
